...
// --- CONFIGURATION ---
// 64 is fine for PC. 
// If you move to Jetson and get lag, try lowering this to 48.
#define MAX_PTS 64
#define EPSILON 1e-6
...
float sdPoly(vec2 p, vec2 pts[MAX_PTS], int count) {
    ...
    // [FIX] Use boolean toggle for winding number (More robust than float multiply)
    bool inside = false;
    ...
    for (int i = 0; i < count; i++) {
        ...
        float ee = dot(e,e);
        // [FIX] Epsilon guard for zero-length segments
    ...
    // Convert boolean to sign (-1.0 is inside)
...
void main() {
    ...
    // SAFETY: If the math glitches, uPadding prevents full-screen artifacts
    ...
    // 3. OUTLINE
    color = mix(color, uColLine, getStroke(dMouth, 4.0));
