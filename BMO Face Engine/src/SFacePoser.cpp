...
void AppendToDatabase(std::string name, FacialParams p) {
    std::ifstream infile("visemes_database.txt");
    std::ofstream tempfile("visemes_database.tmp");
    ...
    // Read through the file and replace if entry exists
    while (std::getline(infile, line)) {
        size_t namePos = line.find("visemes[\"");
        if (namePos != std::string::npos) {
            size_t endPos = line.find("\"]");
            if (endPos != std::string::npos) {
                std::string existingName = line.substr(namePos + 9, endPos - (namePos + 9));
                if (existingName == name) {
                    ...
                    tempfile << "visemes[\"" << name << "\"] = { "
                        ...
                        << p.sigma << "f, " << p.power << "f, " << p.maxLiftValue << "f };\n";
                    found = true;
                    continue;
                }
            }
    ...
    std::remove("visemes_database.txt");
    std::rename("visemes_database.tmp", "visemes_database.txt");
    
    std::cout << (found ? "Updated: " : "Saved: ") << name << std::endl;
}
            ...
            if (parsingEntry && line.find("mouth") != std::string::npos && line.find("{") != std::string::npos) {
                ...
                if (v.size() < 18) {
                    std::cout << "[DB] WARNING on line " << lineNum << " (" << currentItem.name << "): "
                              << "Expected 18 values, found " << v.size() << ". (Using defaults for missing)\n";
                }
                ...
                if(v.size() > 15) currentItem.mouthParams.sigma = v[15];
                if(v.size() > 16) currentItem.mouthParams.power = v[16];
                if(v.size() > 17) currentItem.mouthParams.maxLiftValue = v[17];
            }
...
int main() {
    ...
    Vector2 atlasPos = { rig.centerPos.x, rig.centerPos.y + 0.0f };
    ...
    while (!WindowShouldClose()) {
        ...
        DRAW_SLIDER("Curve",      rig.target.curve,        -2.0f,  2.0f);
        sy += 10.0f;
        DRAW_SLIDER("Sqze Top",   rig.target.squeezeTop,    0.0f,  1.0f);
        DRAW_SLIDER("Sqze Bot",   rig.target.squeezeBottom, 0.0f,  1.0f);
        sy += 10.0f;
        DRAW_SLIDER("Squeeze Sigma",   rig.target.sigma, 0.0f,  1.0f);
        DRAW_SLIDER("Squeeze Power",   rig.target.power, 0.0f,  10.0f);
        DRAW_SLIDER("Squeeze lift",   rig.target.maxLiftValue, 0.0f,  1.0f);
        sy += 10.0f;
        DRAW_SLIDER("Teeth Y",    rig.target.teethY,       -1.0f,  1.0f);
        DRAW_SLIDER("Teeth W",    rig.target.teethWidth,    0.1f,  1.0f);
        ...
        sy += 10.0f;
        DRAW_SLIDER("Position X", rig.centerPos.x, 400.0f, 1200.0f);
        DRAW_SLIDER("Position Y", rig.centerPos.y, 200.0f, 800.0f);

        if (GuiButton({ innerX, sy, innerW, 30 }, "RESET TO NEUTRAL")) {
             rig.target = MakeDefaultParams();
             rig.resetPosition({ 800, 360 });
        }
        sy += 40.0f;
        ...
        // --- [NEW] DATABASE UI SECTION (Right Side) ---
        float rightX = 960.0f;
        float rightY = 20.0f;
        float rightW = 300.0f;

        // Load button
        if (GuiButton({ rightX + 10, rightY + 70, rightW - 20, 30 }, "LOAD SELECTED")) {
        ...
        GuiGroupBox({ rightX, rightY, rightW, 200 }, "VISEME DATABASE");
        
        // Dropdown + Reload button
        Rectangle dropdownRect = { rightX + 10, rightY + 30, rightW - 50, 30 };
        Rectangle reloadRect   = { rightX + rightW - 35, rightY + 30, 30, 30 };
