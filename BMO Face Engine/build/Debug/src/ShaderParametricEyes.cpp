...
struct ParametricEyes {
    ...
    void Init() {
        // [CRITICAL FIX] 
        // Always try to load the SOURCE file first. 
        // This ensures the shader on the GPU matches the code in your editor.
        
        // 1. Resolve Paths (Prioritize src/)
        if (FileExists("src/eyes_es.fs")) eyeFragPath = "src/eyes_es.fs";
        else eyeFragPath = "eyes_es.fs";

        if (FileExists("src/brow_es.fs")) browFragPath = "src/brow_es.fs";
        else browFragPath = "brow_es.fs";

        if (FileExists("src/tears_es.fs")) tearFragPath = "src/tears_es.fs";
        else tearFragPath = "tears_es.fs";
        ...
        // 3. Get Locations (Now guaranteed to work)
        GetEyeLocations();
        GetBrowLocations();
        GetTearLocations();

        // 4. Start Watch Timers
